# OpenHands With Remote Runtime Server

This repository is a fork of OpenHands with a remote runtime server implementation. We deploy our server on Kubernetes, leveraging storage-optimized instances to cache container images and enable fast startup times. To efficiently handle concurrent requests, we leverage [aidocker](https://aiodocker.readthedocs.io/en/latest/) and use the [crun](https://github.com/containers/crun) container runtime, which offers lightweight and high-performance container execution. Our setup supports running approximately 80–100 containers per replica on nodes with just 16 CPUs, without encountering stability issues. While there remains significant room for optimization, our current implementation is simple, effective, and publicly available.
...
We reproduce the original README from OpenHands below:
<a name="readme-top"></a>
