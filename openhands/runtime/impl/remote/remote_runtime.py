...
class RemoteRuntime(ActionExecutionClient):
    ...
    port: int = 88888888  # default port for the remote runtime client
    ...
    def __init__(
        ...
        self.runtime_builder = RemoteRuntimeBuilder(
            ...
            self.session,
            timeout=self.config.sandbox.remote_runtime_api_timeout,
        )
        self.available_hosts: dict[str, int] = {}

    @property
    def api_url(self):
        return self.runtime_builder.api_url
    ...
    def _check_existing_runtime(self) -> bool:
        try:
            # NOTE (sumanthrh): This is a bit messy when the remote runtime is behind a k8s service. The current assumption is
            # that after the first `/image_exists` or the first `/build` request, the RuntimeBuilder will get access to the exact
            # node IP behind the service. This means that for the first time a session is intiated, the client request will get routed to a random node
            # which can be different from the node which handles `/image_exists`. This is fine because it is a new session
            # for all future requests for the same session, the `api_url` parameter will be updated to ensure that routing is correct.
            response = self._send_runtime_api_request(
                'GET',
                f'{self.api_url}/sessions/{self.sid}',
            )
            data = response.json()
            ...
            self.log(
                'error',
                f'Invalid JSON response from runtime API: {e}. URL: {self.api_url}/sessions/{self.sid}. Response: {response}',
            )
            raise
    ...
    def _build_runtime(self) -> None:
        ...
        response = self._send_runtime_api_request(
            'GET',
            f'{self.api_url}/registry_prefix',
        )
    ...
    def _wait_until_alive_impl(self) -> None:
        ...
        runtime_info_response = self._send_runtime_api_request(
            'GET',
            f'{self.api_url}/runtime/{self.runtime_id}',
        )
